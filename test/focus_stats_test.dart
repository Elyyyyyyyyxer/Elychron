...
void main() {
  ...
  group('区间合计', () {
    ...
    test('今天：只算今天的两条', () {
      expect(
        FocusStats.totalBetween(sessions, FocusStats.startOfToday(now),
    ...
    test('本周：周一起算（9/12 是周六，本周从 9/7 开始）', () {
      ...
      expect(
        FocusStats.totalBetween(
            sessions, weekStart, now.add(const Duration(days: 1))),
    ...
    test('本月：9/5 那条也算进来', () {
      expect(
        FocusStats.totalBetween(sessions, FocusStats.startOfMonth(now),
            now.add(const Duration(days: 1))),
  ...
  group('每日合计（柱状图）', () {
    ...
    test('连续七天，没有记录的天也在（值为 0）', () {
      final days =
          FocusStats.daily(sessions, fromDay: DateTime(2026, 9, 6), days: 7);
  ...
  group('按专注对象聚合', () {
    ...
    test('同一任务的会话合并，按时长倒序', () {
      ...
      // 全部历史里旧记录300 分钟最长，其次才是合并后的写报告90 分钟
    ...
    test('可以只看某个时间之后的', () {
      final list =
          FocusStats.byLabel(sessions, from: FocusStats.startOfMonth(now));
  ...
  group('轮数与中断', () {
    ...
    test('没有会话时占比是 null（界面显示还没数据而不是 0%）', () {
  ...
  group('按标签聚合（一条会话计入它的每个标签）', () {
    final sessions = [
      ...
      session(
          ...
          label: '看论文',
          taskUid: 't2'),
      // 放在八月：这样下面只看某个月之后那条能把它过滤掉
      session(
          startedAt: DateTime(2026, 8, 30, 9, 0),
          focusedMinutes: 45,
          label: '敲代码'),
    ];
    final tags = {
  ...
  group('按课程分布', () {
    ...
    test('自由专注与上个月的都不计入', () {
      ...
      final sum =
          list.fold<Duration>(Duration.zero, (acc, item) => acc + item.focused);
