...
/// 原因是划掉声明成了 `showsUserInterface: false`：这种按钮**不会把 App
/// 拉到前台**，响应只会送到后台 isolate，而后台 isolate 碰不到前台界面，
/// 于是通知是取消了，但**全屏闹钟照样响**。延迟提醒是 `true`，所以一直能用。
void main() {
  AndroidNotificationAction actionNamed(
      List<AndroidNotificationAction> list, String id) {
    return list.firstWhere((a) => a.id == id);
  }

  test('★ 闹钟模式下划掉必须走前台（否则闹钟停不下来）', () {
  ...
  test('延迟提醒两种模式都要能拉起 App（它要改时间、落库）', () {
    for (final mode in [
      TaskReminder.modeAlarm,
      TaskReminder.modeNotification
    ]) {
