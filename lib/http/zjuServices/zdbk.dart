...
class Zdbk {
  ...
  List<Session> _parseSessions(Object? raw, String context,
    ...
    // 只记条数，不记课程内容；出问题时一眼能看出是没抓到还是抓到了被过滤掉。
    ...
    try {
      ...
      final bothHalves = sessions
          ...
          .toSet()
          .toList();
      // 靠猜的有多少门， 2026-09-17 加：秋冬混淆的真凶就是"猜出来的半学期"
      ...
      //   要看诊断请走设置 → 诊断与测试 → 测试日志里的 操作=halfDiag 那条。）
    } catch (_) {
      // 诊断本身不能影响课表解析
    }
    return sessions;
  ...
  /// 一次刷新本来要按学年 × 学期查 8~16 次， 历史学年直接吃缓存，
    ...
    return await _withAutoRelogin(httpClient, (relogged, retried) async {
      ...
      try {
        for (var i = 0; i < 3; i++) {
          ...
          // 拿一次成功但 0 行把它覆盖掉，就等于把上一次好数据也弄没了。
          if (items.isNotEmpty) {
