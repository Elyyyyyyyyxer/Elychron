...
/// ============ 循环保护 / 卡死诊断 ============
///
/// 背景：App 出现过用着用着突然卡死，只能重启的情况，而且**没有 ANR 记录、
/// 没有崩溃日志**，这几乎只可能是 **UI 线程上的死循环**（Dart 代码在同一个
/// isolate 里转圈，系统看不到 ANR，但因为一直不返回，界面完全没响应）。
///
/// 项目里有一批理论上有限、但依赖数据正确性的 while 循环（重复日程滚动、
...
///
/// 记下的日志会出现在设置 → 诊断与测试 → 测试日志里，用户可以直接复制出来。
class LoopGuard {
  LoopGuard(this.what, {this.limit = defaultLimit});

  /// 任何应该有限的循环都不该迭代超过这个次数
  ...
  /// 每轮循环开头调用；返回 true 表示已经超限，请立刻 break
  bool tick() {
    _iterations++;
...
/// 给每秒跑一次的逻辑测耗时：慢过阈值就记一条日志，用来说明卡在哪一步
class SlowWatch {
