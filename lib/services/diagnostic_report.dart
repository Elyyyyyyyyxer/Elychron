...
class DiagnosticModuleReport {
  ...
  /// 模块自己报的那句话（例如课表的入库 15 条（已确定 15 条），能显示 0 条）。
  ///
  /// [reason] 是按状态写死的通用说明，看不出抓到了但被过滤掉这种细节，
  /// 所以把模块结果原文一并带出来显示。
  final String? detail;
...
class _ModuleBuilder {
  ...
  void add(DiagnosticLogEntry entry, {required bool expectedFallback}) {
    ...
    if (entry.operation == 'result') {
      ...
      // 模块自己报的最后一句话，原样带到报告里（实时成功这类通用词留着也没用，
      // 真正有用的是入库 15 条…能显示 0 条这种带数字的）。
