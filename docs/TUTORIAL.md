...
图片放 `assets/tutorial/<教程id>/xxx.png`，内容文件里写：

    TutorialImageStep(
      asset: 'assets/tutorial/tasks/types.png',
      title: '新建页顶部的类型选择',   // 图上面的标题，可省
      caption: '选错类型，提醒时间会差很远',  // 图下面的说明，可省
    ),

**不用改 `pubspec.yaml`** —— 它已经声明了整个 `assets/` 目录，新文件自动会被打包。
图还没准备好也没关系：播放器会显示一个占位框并写出期望路径，教程照样能走通。

**一张图一步**。想放多张就多写几步 —— 教程的约定是"一步只说一件事"，
而且截图很长，堆在一屏里没法看。

#### 图在手机上是怎么显示的（2026-09-17 改过）

- 按屏幕宽度**等比缩放**，但**限高**（屏幕高度的 46%）。
  手机截图是 1080×2376 这种竖长条，按宽度铺满会占掉两屏多，用户得一直往下拉才能看到"下一步"。
- 右下角有个 **「点图放大」** 角标：点一下进全屏，可以**捏合/双击放大**看细节 ——
  教程里最常见的需求就是"看清那个按钮到底在哪"，这一步不能省。
- 解码时转圈（截图几百 KB），图缺失时显示占位框。

#### 截图从哪来（怎么自己做一张）

1. 手机连上电脑（`adb connect <ip>:5555`），把 App 切到要截的那个页面；
2. `adb shell screencap -p /sdcard/x.png` + `adb pull /sdcard/x.png`；
3. **裁掉无关部分**（顶部状态栏、底部导航栏、以及**任何个人数据**）；
4. 存成 PNG 放进 `assets/tutorial/<教程id>/`，在内容文件里引用它。

⚠️ **红线：教程图片会跟着源码公开（GPLv3 仓库）。绝不能出现**
姓名、学号、成绩、待办标题、聊天内容这类**个人数据**。
截图前先把演示数据清干净，或者只截"没有任何个人信息的区域"（例如新建页的表单、设置页的开关）。

> 跑一次 `flutter test test/tutorial_test.dart`：
> ① 注册表自查会告诉你 id 有没有重复、哪一步忘了写内容、图片路径是不是写错了；
> ② **还会真的去 `rootBundle` 里加载每一张图** —— 路径写错、文件没放进来，测试直接红。
